...
class Contactform extends Module implements WidgetInterface
{
    ...
    public function sendMessage()
    {
        ...
        $message = '</p><p>'.$this->l('Name'). ': '.trim(Tools::getValue('fname')) .' '. trim(Tools::getValue('lname')). '</p>';
        $message .= '<p>'. $this->l('Phone number'). ': '.trim(Tools::getValue('phonenumber')).' / ';
        $message .= trim(Tools::getValue('message'));
        
        // .'<br/>' . $this->l('Phone number'). ': '.trim(Tools::getValue('phonenumber')) . '<br/>' . trim(Tools::getValue('message'));
        ...
        if (!($from = trim(Tools::getValue('from'))) || !Validate::isEmail($from)) {
            ...
            if (!count($this->context->controller->errors)
                ...
                $var_list = [
                    ...
                    '{order_name}' => '-',
                    '{attached_file}' => '-',
                    '{message}' => Tools::nl2br(Tools::htmlentities(Tools::stripslashes($message))),
                    '{email}' =>  $from,
                    '{product_name}' => '',
                ];
