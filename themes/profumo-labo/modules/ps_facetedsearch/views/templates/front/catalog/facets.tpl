        ...
        {foreach from=$displayedFacets item="facet"}
          ...
          <section class="search-filters__block list-group-item">
            <div class="search-filters__header d-flex justify-content-between align-items-center h5 position-relative">
              ...
              <a href="#facet_{$_expand_id}" class="{$facet['type']} icon-collapse stretched-link text-reset d-block" data-toggle="collapse"
                {if !$_collapse} aria-expanded="true" {/if}>
                <span class="material-icons">&#xE313;</span>
              </a>
            </div>
            ...
            {if in_array($facet.widgetType, ['radio', 'checkbox'])}
              {block name='facet_item_other'}
                <div id="facet_{$_expand_id}" class="{$facet['type']} search-filters__collapse collapse{if !$_collapse} show{/if}">
            ...
            {elseif $facet.widgetType == 'slider'}
              {block name='facet_item_slider'}
                {foreach from=$facet.filters item="filter"}
                  ...
                  <div>
                    <div id="facet_{$_expand_id}" class="{$facet['type']} search-filters__collapse  collapse{if !$_collapse} show{/if} search-filters__slider">
