...
class Dispatcher extends DispatcherCore
{
		...
		if (Configuration::get('SEOURL_CATEGORY')) {
			...
			$this->default_routes[$route_id] = array(
				...
				'keywords' => array(
					...
					'rewrite_category' =>        array('regexp' => '[_a-zA-Z0-9\pL\pS-]*', 'param' => 'rewrite_category'),
					'parents' =>        array('regexp' => '[/_a-zA-Z0-9-\pL]*', 'param' => 'parents'),
				),
			);
		}
		...
		if (Configuration::get('SEOURL_PRODUCT')) {
			...
			$this->default_routes[$route_id] = array(
				...
				'keywords' => array(
					'id' =>            array('regexp' => '[0-9]+'),
					'rewrite' =>        array('regexp' => '[_a-zA-Z0-9\pL\pS-]*', 'param' => 'rewrite'),
					'rewrite_product' =>        array('regexp' => '[_a-zA-Z0-9\pL\pS-]*', 'param' => 'rewrite_product'),
		...
		if (Configuration::get('SEOURL_SUPPLIER')) {
			...
			$this->default_routes['supplier_rule'] = array(
				...
				'keywords' => array(
					'id' =>            array('regexp' => '[0-9]+'),
					'rewrite' =>        array('regexp' => '[_a-zA-Z0-9\pL\pS-]*', 'param' => 'rewrite'),
					'rewrite_supplier' =>        array('regexp' => '[_a-zA-Z0-9\pL\pS-]*', 'param' => 'rewrite_supplier'),
					'meta_keywords' =>    array('regexp' => '[_a-zA-Z0-9-\pL]*'),
					'meta_title' =>        array('regexp' => '[_a-zA-Z0-9-\pL]*'),
				),
			);
		...
		if (Configuration::get('SEOURL_MANUFACTURER')) {
			...
			$this->default_routes['manufacturer_rule'] = array(
				...
				'keywords' => array(
					'id' =>            array('regexp' => '[0-9]+'),
					'rewrite' =>        array('regexp' => '[_a-zA-Z0-9\pL\pS-]*', 'param' => 'rewrite'),
					'rewrite_manufacturer' =>        array('regexp' => '[_a-zA-Z0-9\pL\pS-]*', 'param' => 'rewrite_manufacturer'),
					'meta_keywords' =>    array('regexp' => '[_a-zA-Z0-9-\pL]*'),
					'meta_title' =>        array('regexp' => '[_a-zA-Z0-9-\pL]*'),
				),
			);
		...
		if (Configuration::get('SEOURL_CMS')) {
			...
			$this->default_routes['cms_rule'] = array(
				...
				'keywords' => array(
					'id' =>            array('regexp' => '[0-9]+'),
					'rewrite' =>        array('regexp' => '[_a-zA-Z0-9\pL\pS-]*', 'param' => 'rewrite'),
					'rewrite_cms' =>        array('regexp' => '[_a-zA-Z0-9\pL\pS-]*', 'param' => 'rewrite_cms'),
					'meta_keywords' =>    array('regexp' => '[_a-zA-Z0-9-\pL]*'),
					'meta_title' =>        array('regexp' => '[_a-zA-Z0-9-\pL]*'),
				),
			);
		...
		if (Configuration::get('SEOURL_CMS_CATEGORY')) {
			...
			$this->default_routes['cms_category_rule'] = array(
				...
				'keywords' => array(
					'id' =>            array('regexp' => '[0-9]+'),
					'rewrite' =>        array('regexp' => '[_a-zA-Z0-9\pL\pS-]*', 'param' => 'rewrite'),
					//'rewrite_cms_category' =>        array('regexp' => '[_a-zA-Z0-9\pL\pS-]*', 'param' => 'rewrite_cms_category'),
					'meta_keywords' =>    array('regexp' => '[_a-zA-Z0-9-\pL]*'),
					'meta_title' =>        array('regexp' => '[_a-zA-Z0-9-\pL]*'),
				),
			);
    ...
    protected function setRequestUri()
    {
        ...
        if ($this->use_routes && Language::isMultiLanguageActivated()) {
            ...
            if (preg_match('#^/([a-z]{2})(?:/.*)?$#', $this->request_uri, $m)) {
                if (Configuration::get('SEOURL_REMOVE_DEFAULT_ISO') && !in_array($m[1], array('pl', 'de', 'en', 'it', 'ru'))) {
                    $iso_code = DB::getInstance()->getvalue('SELECT iso_code FROM '._DB_PREFIX_.'lang WHERE iso_code = \''.pSQL($m[1]).'\'');
                } else { // jeśli nie usuwamy domyślnego iso to przypisujemy
                    $iso_code = true;
                }
                ...
                if (strpos($_SERVER['REQUEST_URI'], 'modules/') != false || (isset($_POST['controller']) && $_POST['controller'] == 'cart') || (Tools::getValue('controller') == 'authentication') && Tools::getvalue('ajax')) {
    ...
    protected function loadRoutes($id_shop = null)
    {
		...
		if ($this->module->is_1_5() || !method_exists($language, 'getIDs')) {
			$language_ids = array();
        ...
        if ($this->use_routes) {
            foreach ($this->default_routes as $route_id => $route_data) {
				// If route is without ID
				if (in_array($route_id, array('category_rule', 'supplier_rule', 'manufacturer_rule', 'cms_rule', 'cms_category_rule', 'product_rule'))) {
