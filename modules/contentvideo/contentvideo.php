...
class CONTENTVIDEO extends Module
{
	public function __construct()
	{
		$this->name = 'contentvideo';
		$this->description = 'contentvideo';
		$this->tab = 'front_office_features';
		$this->version = '1.1.3';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.7');
		$this->monolanguage_content = 'Contentvideo_MONOLANGUAGE';
		$this->text_editor_content = 'Contentvideo_TEXTEDITOR';
		$this->content_wrapper = 'Contentvideo_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Contentvideo_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Contentvideo_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('contentvideo');
		$this->description = $this->l('contentvideo');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !CONTENTVIDEOModel::createTables())
			return false;
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = CONTENTVIDEOModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = CONTENTVIDEOModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					CONTENTVIDEOModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					CONTENTVIDEOModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																CONTENTVIDEOModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class CONTENTVIDEOModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( CONTENTVIDEOModel::createFilesTable()
				&& CONTENTVIDEOModel::createContentTable());
	}
