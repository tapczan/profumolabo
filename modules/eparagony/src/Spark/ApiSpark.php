...
use ASoftwareHouse\EParagony\MoneyTool;
...
class ApiSpark
{
    ...
    private function getProducts($rawProducts)
    {
        ...
        foreach ($rawProducts as $rawProduct) {
            ...
            $product->taxRate = $this->taxHelper->decodeToLetter($rawProduct['tax_rate']);
            $product->taxRateValue = (string)MoneyTool::roundTax($rawProduct['tax_rate']); /* Yes, string. */
    ...
    private function getSubtotalsByTaxRate($rawProducts, $rawShipping)
    {
        $subtotalsByTaxRate = [];
        foreach ($rawProducts as $rawProduct) {
            $taxLetter = $this->taxHelper->decodeToLetter($rawProduct['tax_rate']);
    ...
    private function getTotalValue($rawProducts, $rawShipping)
    {
        $withTax = 0;
        foreach($rawProducts as $product) {
            $withTax += MoneyTool::roundToCentile($product['our_total_price']);
    ...
    private function getRecipeLines($rawProducts, $rawShipping)
    {
        ...
        foreach ($rawProducts as $rawProduct) {
            ...
            $line->unitPrice = MoneyTool::displayMoneyWithDot($rawProduct['our_unit_price']);
            $line->totalLineValue = MoneyTool::displayMoneyWithDot($rawProduct['our_total_price']);
            $line->taxRate =  $this->taxHelper->decodeToLetter($rawProduct['tax_rate']);
            if ($rawProduct['our_org_price'] !== $rawProduct['our_unit_price']) {
                ...
                $label = 'Zniżka ' . $rawProduct['reduction_percent'] . '%';
                $line->rebatesMarkups = [[
                    'type' => 'OBNIŻKA',
                    'value' => MoneyTool::displayMoneyWithDot($value),
                    'name' => $label,
    ...
    private function getFinalRequestData(
        ...
        $grossSaleValue = $this->getTotalValue($rawProducts, $rawShipping);
