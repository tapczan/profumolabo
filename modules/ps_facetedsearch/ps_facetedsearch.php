...
class Ps_Facetedsearch extends Module implements WidgetInterface
{
    ...
    protected function getDefaultFilters()
    {
        return [
            ...
            'layered_selection_grade_slider' => [ // Angelo grade slider
                'label' => 'Product grade filter (slider)',
                'slider' => true,
            ],
        ];
    }
    ...
    public function rebuildLayeredStructure()
    {
        ...
        /**
         * Angelo's grade filter; added grade on enum.
         */
        $this->getDatabase()->execute(
            ...
            `type` ENUM(\'category\',\'id_feature\',\'id_attribute_group\',\'quantity\',\'condition\',\'manufacturer\',\'weight\',\'price\',\'grade\') NOT NULL,
    ...
    public function rebuildLayeredCache($productsIds = [], $categoriesIds = [], $rebuildLayeredCategories = true)
    {
        ...
        while ($product = $db->nextRow($result)) {
            ...
            foreach ($c as $idCategory => $category) {
                 ...
                 */
                if (!isset($doneCategories[(int) $idCategory]['g'])) {
                    $filterData['layered_selection_grade_slider'] = ['filter_type' => Converter::WIDGET_TYPE_CHECKBOX, 'filter_show_limit' => 0];
                    $doneCategories[(int) $idCategory]['g'] = true;
                    $toInsert = true;
                }
                ...
                foreach ($data['categories'] as $idCategory) {
                    ...
                    foreach ($data as $key => $value) {
                        if (substr($key, 0, 17) == 'layered_selection') {
                            ...
                            } elseif ($key == 'layered_selection_grade_slider') {
                                /**
                                 * Angelo's grade filter
                                 */
                                $sqlInsert .= '(' . (int) $idCategory . ', ' . (int) $idShop . ', NULL,\'grade\',' . (int) $n . ', ' . (int) $limit . ', ' . (int) $type . '),';
...
\'id_feature\',' . (int) $n . ', ' . (int) $limit . ', ' . (int) $type . '),';
    ...
    {
        $this->getDatabase()->execute('DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'layered_average_grade`');

        $this->getDatabase()->execute(
            'CREATE TABLE `' . _DB_PREFIX_ . 'layered_average_grade`
                SELECT 
                    p.id_product, AVG(IFNULL(pc.grade, 0)) AS grade
                FROM
                     `'. _DB_PREFIX_ . 'product` p
                    LEFT JOIN
                    `'. _DB_PREFIX_ . 'product_comment` pc ON (pc.id_product = p.id_product)
                    WHERE pc.validate = 1
                GROUP BY p.id_product;
        ');
    }
