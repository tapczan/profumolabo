...
class Search
{
    ...
    private function addSearchFilters($selectedFilters, $parent, $idShop)
    {
        ...
        foreach ($selectedFilters as $key => $filterValues) {
            ...
            switch ($key) {
                 ...
                 */
                case 'grade':
                    if (!empty($selectedFilters['grade'][0]) || !empty($selectedFilters['grade'][1])) {
                        $this->addGradeFilter(
                            (int) $selectedFilters['grade'][0],
                            (int) $selectedFilters['grade'][1]
                        );
     ...
     * @param $maxGrade
     */
    private function addGradeFilter($minGrade, $maxGrade)
    {
        $this->getSearchAdapter()->addFilter('grade', [$maxGrade], '<=');
        $this->getSearchAdapter()->addFilter('grade', [$minGrade], '>=');
    }
