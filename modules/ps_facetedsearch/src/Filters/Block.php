...
class Block
{
        ...
        foreach ($filters as $filter) {
            switch ($filter['type']) {
                ...
                case 'weight':
                    ...
                    break;
                /**
                 * Angelo's grade filter block
                 */
                case 'grade':
                    $filterBlocks[] = $this->getGradeRangeBlock($filter, $selectedFilters, $nbProducts);
                    break;
                case 'condition':
     ...
     * @return array
     */
    private function getGradeRangeBlock($filter, $selectedFilters, $nbProducts)
    {
        $gradeBlock = [
            'type_lite' => 'grade',
            'type' => 'grade',
            'id_key' => 0,
            'name' => $this->context->getTranslator()->trans('Grade', [], 'Modules.Facetedsearch.Shop'),
            'max' =>  5,
            'min' => 1,
            'unit' => null,
            'specifications' => null,
            'filter_show_limit' => (int) 0,
            'filter_type' => Converter::WIDGET_TYPE_SLIDER,
            'value' => null,
            'nbr' => $nbProducts,
        ];

        $gradeBlock['value'] = !empty($selectedFilters['grade']) ? $selectedFilters['grade'] : null;
