...
class PROFUMOLABOCONTENT extends Module
{
	public function __construct()
	{
		$this->name = 'profumolabocontent';
		$this->description = 'profumolabocontent';
		$this->tab = 'front_office_features';
		$this->version = '1.1.3';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.7');
		$this->monolanguage_content = 'Profumolabocontent_MONOLANGUAGE';
		$this->text_editor_content = 'Profumolabocontent_TEXTEDITOR';
		$this->content_wrapper = 'Profumolabocontent_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Profumolabocontent_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Profumolabocontent_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('profumolabocontent');
		$this->description = $this->l('profumolabocontent');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !PROFUMOLABOCONTENTModel::createTables())
			return false;
	...
	public function uninstall()
	{
		$this->_clearCache('template.tpl');
		if (!parent::uninstall()
			|| !PROFUMOLABOCONTENTModel::DropTables())
		{
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = PROFUMOLABOCONTENTModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = PROFUMOLABOCONTENTModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					PROFUMOLABOCONTENTModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					PROFUMOLABOCONTENTModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																PROFUMOLABOCONTENTModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class PROFUMOLABOCONTENTModel extends ObjectModel
{

	public static $definition = array(
		'table' => 'profumolabocontent',
		'primary' => 'file_id',
		'multishop' => true,
		'multilang' => true,
		'fields' => array(
	...
	public static function createTables()
	{
		...
		return ( PROFUMOLABOCONTENTModel::createFilesTable()
				&& PROFUMOLABOCONTENTModel::createContentTable());
	}
