...
class ArProductViews extends ObjectModel
{
    const TABLE_NAME = 'arproductlist_views';
    ...
    public static function viewProduct($id_product)
    {
        $ip = Tools::getRemoteAddr();
        self::cleanUp();
        return Db::getInstance()->Execute("INSERT INTO `" . self::getTableName() .
            "` (id_view, id_product, ip, view_date) VALUES (NULL, " . (int)$id_product . ", '" .
            pSQL($ip) . "', '" . pSQL(date('Y-m-d H:i:s')) . "')");
    }
    
    public static function cleanUp()
    {
        $ttl = (int)Configuration::get('ARPLG_VIEW_CLEANUP_AFTER');
        if ($ttl == 0) {
            return ;
        }
