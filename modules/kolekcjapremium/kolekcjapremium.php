...
class KOLEKCJAPREMIUM extends Module
{
	public function __construct()
	{
		$this->name = 'kolekcjapremium';
		$this->description = 'kolekcjapremium';
		$this->tab = 'front_office_features';
		$this->version = '1.1.3';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.7');
		$this->monolanguage_content = 'Kolekcjapremium_MONOLANGUAGE';
		$this->text_editor_content = 'Kolekcjapremium_TEXTEDITOR';
		$this->content_wrapper = 'Kolekcjapremium_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Kolekcjapremium_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Kolekcjapremium_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('kolekcjapremium');
		$this->description = $this->l('kolekcjapremium');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !KOLEKCJAPREMIUMModel::createTables())
			return false;
	...
	public function uninstall()
	{
		$this->_clearCache('template.tpl');
		if (!parent::uninstall()
			|| !KOLEKCJAPREMIUMModel::DropTables())
		{
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = KOLEKCJAPREMIUMModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = KOLEKCJAPREMIUMModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					KOLEKCJAPREMIUMModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					KOLEKCJAPREMIUMModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																KOLEKCJAPREMIUMModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class KOLEKCJAPREMIUMModel extends ObjectModel
{

	public static $definition = array(
		'table' => 'kolekcjapremium',
		'primary' => 'file_id',
		'multishop' => true,
		'multilang' => true,
		'fields' => array(
	...
	public static function createTables()
	{
		...
		return ( KOLEKCJAPREMIUMModel::createFilesTable()
				&& KOLEKCJAPREMIUMModel::createContentTable());
	}
